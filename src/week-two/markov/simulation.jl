...
function discrete_sim(
    P::Matrix{Float64}, partition::Partition, T::Int; 
    drop::Int=2000)

    if drop > T throw(ErrorException("Variables to drop ($drop) > total ($total)")) end

    inverse_F = make_inv_F(pseudoF(P))

    evolution_jdx = zeros(Int, T)
    evolution_jdx[1] = rand(1:length(partition))
    ...
    return currygetindex(partition).(evolution_jdx)
